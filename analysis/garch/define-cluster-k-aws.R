...
PseudoF <- function(dataID){
  ...
  ch <- c()
  for(k in 2:15){
    medoid <- matrix(nrow = k,ncol = ncol(D))
    rownames(medoid) <- 1:k
    colnames(medoid) <- colnames(D)
    cluster <- hcut(D, k = k, hc_func = 'hclust', hc_method = 'ward.D2', hc_metric = 'euclidean')$cluster
    for(i in 1:k){
      if(is.null(nrow(D[names(cluster[cluster==i]),]))){
        medoid[i,] <- D[names(cluster[cluster==i]),]
      }
      else{
        medoid[i,] <- apply(D[names(cluster[cluster==i]),],2,mean)
      }
    }
    within.se <- 0
    for(i in 1:k){
      within.i.se <- 0
      for(j in names(cluster[cluster==i])){
        within.i.se <- within.i.se + dist(rbind(D[j,],medoid[i,]), method = 'euclidean')**2
      }
      within.se <- within.se + within.i.se
    }
    within.se <- within.se / (length(cluster) - k)
    
    medoid.all <- apply(D,2,mean)
    without.se <- 0
    for(i in 1:k){
      without.se <- without.se + length(cluster[cluster==i]) * dist(rbind(medoid[i,],medoid.all), method = 'euclidean')**2
    }
    without.se <- without.se / (k-1)
    
    ch[k-1] <- without.se / within.se
  }
  
  plot(2:15, ch, type = 'o', xlab = 'Num of cluster', ylab = 'Pseudo F', col = 'blue')
}
